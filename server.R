...
server <- function(input,output,session) { 
  ...
  output$api_data<-renderDataTable({
    if(input$Category=="All"){
      # query species data from api connection
      base_api<-"https://apiv3.iucnredlist.org/api/v3/"
      token<-"?token=1ac5642498c8cf69b5d5367a6c17711bbdfe48e3beb4ebaa038fd825679eb294"
      
      #Global assessments Indexed by page number
      ...
      # convert data format into R list
      Global_assessments<-fromJSON(data_Input)
      
      # convert to R data.frame to generate a new list of global species
      ...
      # convert to R data.frame to generate a new list of categoryspecies
  ...
  output$Species_Assessed<-renderValueBox({
    valueBox(value=NULL,
             subtitle=" This shiny app will show interactive visualization of IUCN Red list dataset.
             IUCN (International Union for Conservation of Nature) is 
             a membership Union of government and civil society organisations. 
             The IUCN Red List is a critical indicator of the health of 
             the world’s biodiversity. Far more than a list of species and their status, 
             it is a powerful tool to inform and catalyse action for biodiversity conservation and policy change, 
             critical to protecting the natural resources we need to survive. It provides information about range, 
             population size, habitat and ecology, use and/or trade, threats, 
  ...
  output$distribution_Plot <- renderPlotly({
    ...
    observeEvent(input$Species_regional_category, {
      if("ALL" %in% input$Species_regional_category) {
        # If "Select All" is selected, update the input to include all categories
        updateCheckboxGroupInput(session, "Species_regional_category",
                                 selected = c("EX", "EW", "CR", "EN", "VU", "NT.or.LR.nt", "LC.or.LR.lc", "DD"))
      }
    }, ignoreInit = TRUE)
    ...
    # sort data
    # Filter & sort RL categories as required
